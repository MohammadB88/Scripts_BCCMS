...
#k = 0
##l = 0
#for c in np.arange(0.0, Lz + Lz/1000, Lz/1000):
    #print(abs(c - z[j]))
    #print(c, z[j])
    #if abs( c - z[j]) <= 1 :
        ##print(abs( c - z[j]))
        ...
        #l = 0
        #i = 0
        #for a in np.arange(0.0, Lx + Lx/100, Lx/100):
            #if abs(a - x[i]) <= 1 :
                #print(i,j)
                ##print(abs(a - x[i]))
                ##print(a, x[i])
...
#plt.imshow(x,y, disp)
#plt.hist2d(x,y, weights = disp, bins=[100,100], cmap=plt.cm.jet, cmin = min(disp) , cmax = max(disp))

#plt.pcolormesh(x,y,disp_new.T)
